...
async function generateQuote() {
  ...
  merge(`A${summaryRow}:C${summaryRow}`, `Código placa: ${quote.value.plate || 'No informado'}`, { bold: true })
...
<template>
  <main :class="['app-shell', { dark: darkMode }]">
    <header class="nav">
      ...
      <button class="brand" @click="goHome">Quote-generator</button>
    ...
    <section v-else class="quote-page">
      ...
      <form @submit.prevent="openConfirmation">
        ...
        <div class="details-grid">
          <section class="form-card business-card"><div class="field-row"><label>Empresa a la cual va dirigida<input v-model.trim="quote.company" placeholder="Ej. Soluciones S.A.S" required></label><label>Ciudad de la empresa (opcional)<input v-model.trim="quote.city" placeholder="Ej. Bogotá D.C"></label></div><label>Código placa<input v-model.trim="quote.plate" placeholder="ABC-123" required></label></section>
    ...
    <div v-if="showConfirmation" class="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="confirmation-title" @click.self="showConfirmation = false">
      <section class="confirmation-modal">
        ...
        <div class="modal-details"><p><span>Empresa</span><strong>{{ quote.company || 'Sin especificar' }}</strong></p><p><span>N.º cotización</span><strong>{{ quote.number }}</strong></p><p><span>Fecha</span><strong>{{ quote.date || 'Sin especificar' }}</strong></p><p><span>Código placa</span><strong>{{ quote.plate || 'Sin especificar' }}</strong></p></div>
    ...
    <footer><button class="brand" @click="goHome">Quote-generator</button><div><a href="#">Privacy Policy</a><a href="#">Terms of Service</a><a href="#">Support</a></div><span>Desarrollado por Ludwing Hernandez</span></footer>
  </main>
</template>
